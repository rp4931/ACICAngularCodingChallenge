...
export class LineOfBusinessService {
  ...
  getTopTwoLinesOfBusiness(): LineOfBusiness[] {
    const linesOfBusiness = this.dataService.createDb().linesOfBusiness;
    const recentQuotes = this.dataService.createDb().recentQuotes;
    const topTwoLinesOfBusiness = linesOfBusiness
      .sort((a, b) => {
        const aQuotes = recentQuotes.filter(
          (quote) => quote.lineOfBusiness === a.id
        ).length;
        const bQuotes = recentQuotes.filter(
          (quote) => quote.lineOfBusiness === b.id
        ).length;
